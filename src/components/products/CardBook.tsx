...
export const CardBook = ({ img, title, speciality, type, fragment, fileUrl, onViewDetails, onShowDetails, onReserve, cantidadDisponible, hasActiveOrder = false, isAuthenticated = false }: Props) => {
	...
	const isFisico = type === 'Fisico' || type === 'Físico' || type === 'Fisico y Virtual';
	...
	const handleViewDetails = () => {
		...
		// Para libros de tipo "Fisico" (solo físico), mostrar modal especial
		if (type === 'Fisico' || type === 'Físico') {
			if (onShowDetails) {
				onShowDetails();
			}
			return;
		...
		// Para otros tipos, mostrar PDF si existe
