    ...
    <div className="max-w-7xl mx-auto">
      ...
      {usuariosPendientes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {usuariosPendientes.map((usuario) => (
            <div
              key={usuario.id}
              className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500 overflow-hidden flex flex-col min-h-[300px]"
            >
              <div className="flex items-start justify-between mb-4 flex-shrink-0">
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="font-bold text-lg text-gray-800 truncate">{usuario.nombre}</h3>
              ...
              <div className="space-y-2 mb-4 flex-1 overflow-y-auto">
                {usuario.correo && (
                  <div className="flex items-start text-sm text-gray-600">
                    ...
                    <span className="break-words">{usuario.correo}</span>
                  </div>
                )}
                {usuario.telefono && (
                  <div className="flex items-center text-sm text-gray-600">
                    <FaPhone className="mr-2 text-gray-400 flex-shrink-0" />
                    <span className="break-words">{usuario.telefono}</span>
                  </div>
                )}
                {usuario.cedula && (
                  <div className="flex items-center text-sm text-gray-600">
                    <FaIdCard className="mr-2 text-gray-400 flex-shrink-0" />
                    <span className="break-words">{usuario.cedula}</span>
                  </div>
                )}
                {usuario.condominios && (
                  <div className="flex items-start text-sm text-gray-600">
                    <FaBuilding className="mr-2 text-gray-400 flex-shrink-0 mt-0.5" />
                    <span className="break-words">{usuario.condominios.nombre}</span>
                  </div>
                )}
              </div>

              <div className="flex space-x-2 mt-auto pt-4 flex-shrink-0 justify-center">
                <button
                  onClick={() => handleAprobar(usuario.id, 'residente')}
                  disabled={procesandoId === usuario.id}
                  className="bg-green-500 text-white px-6 py-2 rounded hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 text-sm font-medium"
                >
                  <FaCheck />
                  <span>Aprobar</span>
                </button>
                <button
                  ...
                  disabled={procesandoId === usuario.id}
                  className="bg-red-500 text-white px-6 py-2 rounded hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 text-sm font-medium"
                >
                  <FaTimes />
                  <span>Rechazar</span>
                </button>
      ...
      {usuarioRechazoId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto my-auto">
            <h2 className="text-xl font-bold mb-4 text-gray-900">Rechazar Usuario</h2>
            ...
            <textarea
              ...
              className="w-full bg-white text-gray-900 border border-gray-300 rounded p-2 mb-4 min-h-[100px] max-h-[200px] resize-y focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
