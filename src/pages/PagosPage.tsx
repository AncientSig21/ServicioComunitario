...
import { descargarReciboOficial, DatosRecibo } from '../components/recibos/ReciboOficial';
...
interface Pago {
  ...
  recibo_oficial?: DatosRecibo | null; // Datos del recibo oficial generado tras validación
}
...
export const PagosPage = () => {
      ...
      const pagosMapeados = pagosData.map((p: any) => {
        try {
          ...
          return {
            ...
            recibo_oficial: p.recibo_oficial || null, // Datos del recibo oficial si existe
          };
        } catch (err) {
  ...
  // Función para descargar el recibo oficial de un pago validado
  const handleDescargarRecibo = async (pago: Pago) => {
    try {
      const reciboData = (pago as any).recibo_oficial;
      
      if (!reciboData) {
        // Si no hay recibo en memoria, intentar obtenerlo de la BD
        const { data: pagoActualizado, error } = await supabase
          .from('pagos')
          .select('recibo_oficial')
          .eq('id', pago.id)
          .single();
        
        if (error || !pagoActualizado?.recibo_oficial) {
          alert('Este pago no tiene un recibo oficial disponible. Contacte a la administración.');
          return;
        }
        
        await descargarReciboOficial(pagoActualizado.recibo_oficial as DatosRecibo);
      } else {
        await descargarReciboOficial(reciboData as DatosRecibo);
      }
    } catch (error) {
      console.error('Error descargando recibo:', error);
      alert('No se pudo descargar el recibo. Intente nuevamente.');
    }
  };
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 flex flex-wrap gap-2">
                  ...
                  {(() => {
                    ...
                    // Para otros estados (pagado, rechazado, etc.), mostrar botones correspondientes
                    if (pago.estado === 'pagado' || pago.estado === 'parcial' || pago.estado === 'rechazado') {
                      return (
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {/* Botón de Descargar Recibo Oficial - Solo para pagos validados */}
                          {pago.estado === 'pagado' && (pago as any).recibo_oficial && (
                            <button 
