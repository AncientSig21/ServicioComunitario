...
export const validarPago = async ({
  ...
  try {
    ...
    // 8. GENERAR RECIBO OFICIAL: Si el pago fue validado como 'pagado', guardar datos del recibo
    let datosRecibo = null;
    if (estado_final === 'pagado') {
      try {
        // Obtener datos completos del usuario y vivienda
        const { data: usuarioData } = await supabase
          .from('usuarios')
          .select('id, nombre, correo, condominios(nombre)')
          .eq('id', pago.usuario_id)
          .single();
        ...
        const { data: viviendaData } = await supabase
          ...
          .eq('id', pago.vivienda_id)
          .single();

        // Obtener nombre del admin que validó
        ...
        // Generar UUID único para el recibo
        const idTransaccion = `TXN-${Date.now()}-${Math.random().toString(36).substring(2, 9).toUpperCase()}`;
        const numeroRecibo = `REC-${new Date().getFullYear()}-${String(pago_id).padStart(6, '0')}`;
        const fechaValidacion = getCurrentLocalISOString();

        // Obtener tasa del dólar actual para incluir en el recibo
        let tasaDolarActual: number | null = null;
        try {
          tasaDolarActual = await getTasaParaCalculo();
        } catch (tasaErr) {
          console.warn('Error obteniendo tasa del dólar para recibo:', tasaErr);
        }

        // Construir objeto de datos del recibo (inmutable)
        datosRecibo = {
          id_transaccion: idTransaccion,
          numero_recibo: numeroRecibo,
          fecha_validacion: fechaValidacion,
          fecha_pago_original: pago.fecha_pago || fechaValidacion,
          residente_nombre: usuarioData?.nombre || 'N/A',
          residente_apartamento: viviendaData?.numero_apartamento || 'N/A',
          residente_condominio: (usuarioData?.condominios as any)?.nombre || 'N/A',
          concepto: pago.concepto,
          monto_bs: nuevo_abono > 0 ? nuevo_abono : monto_total,
          monto_usd: pago.monto_usd || null,
          referencia_usuario: referencia || pago.referencia || null,
          metodo_pago: metodo_pago || pago.metodo_pago || null,
          tasa_dolar: tasaDolarActual,
          validado_por: adminData?.nombre || `Admin #${admin_id}`,
          observaciones: observaciones || null,
          // Metadatos de seguridad
          generado_en: fechaValidacion,
          inmutable: true
        };

        // Guardar datos del recibo en la columna recibo_oficial (JSON)
        const { error: reciboError } = await supabase
          .from('pagos')
          .update({
            recibo_oficial: datosRecibo,
            updated_at: getCurrentLocalISOString()
          })
          .eq('id', pago_id);

        if (reciboError) {
          console.warn('Error guardando datos del recibo oficial:', reciboError.message);
          // No es crítico, el pago ya fue validado
        } else {
          console.log(`Recibo oficial generado para pago ${pago_id}: ${numeroRecibo}`);
        }
      } catch (reciboErr: any) {
        console.warn('Error generando recibo oficial (no crítico):', reciboErr);
      }
    }
...
export const crearPagosMasivos = async ({
  ...
  distribuir_monto?: boolean; // Si es true, el monto total se divide entre los usuarios seleccionados
  es_pago_fijo?: boolean; // Si es true, marca el pago como gasto fijo para seguimiento
}) => {
  try {
    ...
    // =====================================================================
    // PAGO GENERAL DISTRIBUIDO: Calcular monto por persona si está activo
    // =====================================================================
    const cantidadUsuarios = usuariosObjetivo.length;
    let montoCalculadoPorPersona: number;
    let montoUsdPorPersona: number | null = null;

    // Generar ID único para agrupar pagos de un mismo gasto fijo distribuido
    // Este ID permite rastrear todos los pagos que pertenecen al mismo "grupo" de distribución
    const gastoFijoGrupoId = (es_pago_fijo && distribuir_monto) 
      ? `gf-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
      : null;

    if (distribuir_monto && cantidadUsuarios > 0) {
      // Modo distribuido: el monto total se divide entre todos los usuarios
      montoCalculadoPorPersona = Math.round((montoFinal / cantidadUsuarios) * 100) / 100;
      if (montoUsdInsert != null) {
        montoUsdPorPersona = Math.round((montoUsdInsert / cantidadUsuarios) * 100) / 100;
      }
      console.log(`Modo distribuido activo: ${montoFinal} Bs / ${cantidadUsuarios} usuarios = ${montoCalculadoPorPersona} Bs por persona`);
      if (gastoFijoGrupoId) {
        console.log(`Gasto Fijo Grupo ID: ${gastoFijoGrupoId}`);
      }
    } else {
      // Modo normal: cada usuario paga el monto completo
      montoCalculadoPorPersona = montoFinal;
      montoUsdPorPersona = montoUsdInsert;
    }

    // Construir observaciones con información adicional
    let observacionesBase = `Pago creado masivamente por administrador`;
    if (distribuir_monto) {
      observacionesBase += ` | DISTRIBUIDO: Total ${montoFinal} Bs entre ${cantidadUsuarios} usuarios`;
    }
    if (es_pago_fijo) {
      observacionesBase += ` | GASTO FIJO`;
    }
    ...
    const pagosACrear = usuariosObjetivo.map(usuario_id => {
      const obj: any = {
        ...
        concepto,
        monto: montoCalculadoPorPersona, // Usamos el monto calculado (distribuido o completo)
        tipo: tipoMapeado,
        estado: 'pendiente' as EstadoEnum,
        fecha_vencimiento: fecha_vencimiento || null,
        fecha_pago: null,
        ...
        abono: 0,
        es_pago_fijo: es_pago_fijo || false, // Flag de gasto fijo
        // Campos para agrupar y rastrear gastos fijos distribuidos
        gasto_fijo_grupo_id: gastoFijoGrupoId, // Identificador del grupo (null si no es gasto fijo distribuido)
        monto_total_distribuido: (es_pago_fijo && distribuir_monto) ? montoFinal : null, // Meta original del gasto
        monto_usd_total_distribuido: (es_pago_fijo && distribuir_monto && montoUsdInsert) ? montoUsdInsert : null,
        cantidad_usuarios_distribucion: (es_pago_fijo && distribuir_monto) ? cantidadUsuarios : null,
        created_at: getCurrentLocalISOString(),
        updated_at: getCurrentLocalISOString()
      };
      if (montoUsdPorPersona != null) obj.monto_usd = montoUsdPorPersona;
      return obj;
    });
    ...
    // Si falla (ej. columnas condominio_id/abono/vivienda_id/es_pago_fijo no existen), reintentar solo con campos básicos
    if ((!pagosCreados || pagosCreados.length === 0) && insertError) {
      console.warn('Insert con todos los campos falló, reintentando sin condominio_id/vivienda_id/abono/es_pago_fijo:', insertError.message);
      const pagosMinimos: any[] = usuariosObjetivo.map(usuario_id => ({
        usuario_id,
        concepto,
        monto: montoCalculadoPorPersona, // Usar el monto calculado (distribuido o completo)
        tipo: tipoMapeado,
        estado: 'pendiente' as const,
        fecha_vencimiento: fecha_vencimiento || null,
        observaciones: observacionesBase,
        // Preservar campos de gasto fijo en el fallback
        es_pago_fijo: es_pago_fijo || false,
        gasto_fijo_grupo_id: gastoFijoGrupoId,
        monto_total_distribuido: (es_pago_fijo && distribuir_monto) ? montoFinal : null,
        monto_usd_total_distribuido: (es_pago_fijo && distribuir_monto && montoUsdInsert) ? montoUsdInsert : null,
        cantidad_usuarios_distribucion: (es_pago_fijo && distribuir_monto) ? cantidadUsuarios : null,
        created_at: getCurrentLocalISOString(),
        updated_at: getCurrentLocalISOString()
      }));
      if (montoUsdPorPersona != null) pagosMinimos.forEach(p => { p.monto_usd = montoUsdPorPersona; });
    ...
    if (pagosCreados && pagosCreados.length > 0) {
      const historiales = pagosCreados.map((pago: any) => ({
        ...
        datos: { 
          ...
          monto_por_persona: montoCalculadoPorPersona,
          monto_usd_original: montoUsdInsert ?? undefined,
          monto_usd_por_persona: montoUsdPorPersona ?? undefined,
          total_usuarios: cantidadUsuarios,
          distribuido: distribuir_monto || false,
          es_pago_fijo: es_pago_fijo || false,
          gasto_fijo_grupo_id: gastoFijoGrupoId ?? undefined
        },
        fecha_evento: getCurrentLocalISOString()
      }));
    ...
    // Notificar a los usuarios afectados (mostrar el monto que le corresponde a cada uno)
    const montoTextoNotificacion = distribuir_monto 
      ? (montoUsdPorPersona != null 
          ? `${montoUsdPorPersona.toFixed(2)} USD (≈ ${montoCalculadoPorPersona.toFixed(2)} Bs) - Monto distribuido`
          : `${montoCalculadoPorPersona.toFixed(2)} Bs - Monto distribuido`)
      : (montoUsdInsert != null 
          ? `${monto_usd} USD (≈ ${montoFinal} Bs)` 
          : `${montoFinal} Bs.`);
    for (const usuario_id of usuariosObjetivo) {
      await crearNotificacion(
        usuario_id,
        'pago_creado',
        `Se ha creado un nuevo pago: ${concepto} - ${montoTextoNotificacion}${es_pago_fijo ? ' (Gasto Fijo)' : ''}`,
        'pagos',
        null
      );
    }
    ...
    return {
      ...
      // Información adicional para Pago General Distribuido
      distribuido: distribuir_monto || false,
      es_pago_fijo: es_pago_fijo || false,
      monto_total_original: montoFinal,
      monto_por_persona: montoCalculadoPorPersona,
      monto_usd_por_persona: montoUsdPorPersona,
      // Identificador del grupo para rastreo en Centro de Recaudación
      gasto_fijo_grupo_id: gastoFijoGrupoId
    };
  } catch (error) {
...
export const fetchAnuncios = async (
  ...
  try {
    // Intentar con join a archivos para obtener la imagen
    let query = supabase
      .from('anuncios')
      .select(`
        *,
        autor_usuario:usuarios!anuncios_autor_usuario_id_fkey(nombre, correo, rol),
        archivo_imagen:archivos!anuncios_archivo_imagen_id_fkey(id, url, nombre_original)
      `)
    ...
    // Si falla el join a archivos, intentar sin él
    if (error) {
      console.warn('fetchAnuncios: Fallo con join a archivos, intentando sin join...');
      ...
      if (!incluirInactivos) {
        querySimple = querySimple.eq('activo', true);
      }
      if (condominioId != null) {
        querySimple = querySimple.eq('condominio_id', condominioId);
      }
      if (categoria) {
        querySimple = querySimple.eq('categoria', categoria);
      }

      const result = await querySimple;
      data = result.data;
      error = result.error;
    }
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        activo: activo ?? true,
        archivo_imagen_id: archivo_imagen_id ?? null,
    ...
    // Asociar el archivo de imagen con el anuncio
    if (archivo_imagen_id && data && data[0]) {
      await supabase
        .from('archivos')
        .update({ entidad_id: data[0].id })
        .eq('id', archivo_imagen_id);
    }
...
// Subir imagen de anuncio/evento usando la tabla archivos (base64)
export const subirImagenAnuncio = async (file: File, usuario_id: number): Promise<number | null> => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        usuario_id,
        entidad: 'anuncios',
        entidad_id: null, // Se actualizará después de crear el anuncio
        nombre_original: file.name,
        tipo_mime: file.type,
        url: base64Data,
        created_at: getCurrentLocalISOString()
    ...
    console.error('Error subiendo imagen de anuncio:', error);
    throw error;
  }
};

// Obtener imagen de anuncio por ID de archivo
export const obtenerImagenAnuncio = async (archivoId: number): Promise<string | null> => {
  try {
    const { data, error } = await supabase
      .from('archivos')
      .select('url')
    ...
    console.error('Error obteniendo imagen de anuncio:', error);
    return null;
  }
};
